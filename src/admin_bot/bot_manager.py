...
from src.common.models import GroupFamily, Employee, User
from src.common.models import SubscriptionRequest
from src.common.utils import get_inline_keyboard, format_message
...
class BotManager(Commands):
    ...
    def _start_scheduler(self):
        ...
        self.application.job_queue.run_daily(
            self._kick_subgroup_users_not_in_main_group,
            time=datetime.time(23, 30, tzinfo=timezone(django_settings.TIME_ZONE))
        )
    ...
    async def _resolve_subscription_requests(self, context: ContextTypes.DEFAULT_TYPE):
        ...
        if not await requests.aexists():
            return

        request: SubscriptionRequest = await requests.afirst()

        async for request in requests:
            try:
                bot_owner_ids = [user['id'] for user in settings.BOT_OWNERS]
                admin_ids = [employee.id async for employee in Employee.objects.filter(role=EmployeeRole.ADMIN)]
                owner_and_admin_ids = bot_owner_ids + admin_ids
                ...
                for user_id in owner_and_admin_ids:
                    try:
                        await context.bot.send_message(
                            user_id,
                            text=message,
